...
void make_throw(GtkWidget *widget, gpointer data) {
  ...
  const int dice_type =
      (int)gtk_drop_down_get_selected(GTK_DROP_DOWN(fields->dice_type));
  ...
  sprintf(throw_result, "%d (%d%s+%d)",
          calculate_average_throw(dices, &DiceArray[dice_type]) + modificator,
          dices, *(&DiceArray[dice_type].dice_type), modificator);
...
void configurate_app(GtkApplication *app, gpointer used_data) {
  ...
  g_signal_connect(GTK_BUTTON(roll_button), "clicked", G_CALLBACK(make_throw),
                   &fields);
...
int init_app(int argc, char **argv) {
  GtkApplication *app =
      gtk_application_new("Dice.Roller", G_APPLICATION_DEFAULT_FLAGS);
